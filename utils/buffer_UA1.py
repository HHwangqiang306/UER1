...
class Buffer:
    ...
    def __init__(self, buffer_size, device):
        ...
        self.attributes = ['examples', 'labels', 'logits', 'mean_stds', 'task_labels']
        self.dict = {}
        ...
        self.liujiayi = torch.ones(self.buffer_size).to(self.device) * -float('inf')

    def init_tensors(self, examples: torch.Tensor, labels: torch.Tensor,
                     logits: torch.Tensor, mean_stds:torch.Tensor, task_labels: torch.Tensor) -> None:
        """
    ...
    def merge_scores(self):
        ...
        presoftscores = self.importance_scores* self.balance_scores
        #presoftscores = self.importance_scores
        #dongtaibianhuan

        if presoftscores.max() - presoftscores.min() != 0:
            presoftscores_norm = math.sqrt(presoftscores.var())
            presoftscores1 = (presoftscores - presoftscores.mean()) / (presoftscores_norm)
            presoftscores2 = presoftscores1.abs()
        self.scores = presoftscores2 / presoftscores2.sum()
    ...
    def add_data(self, examples, labels=None, logits=None, mean_stds=None, task_labels=None, loss_scores=None):
        """
        Adds the data to the memory buffer according to the reservoir strategy.
        :param examples: tensor containing the images
        :param labels: tensor containing the labels
        :param mean_stds: tensor containing the mean_stds
        :param logits: tensor containing the outputs of the network
        :param task_labels: tensor containing the task labels
        :return:
        """
        self.init_tensors(examples, labels, logits, mean_stds, task_labels)
        ...
        for i in range(examples.shape[0]):
            ...
            if index >= 0:
                ...
                if mean_stds is not None:
                    self.mean_stds[index] = mean_stds[i].to(self.device)
    ...
    def get_data(self, size: int, transform: transforms=True, return_indexes=False) -> Tuple:
        """
        ...
        if transform is True: transform = transforms.RandomHorizontalFlip()
    ...
    def get_all_data(self, transform: transforms=True) -> Tuple:
        """
